...
def video_devices() -> list[str]:
    ...
    for path in glob.glob("/dev/video*"):
        name = path.removeprefix("/dev/video")

        if name.isdigit():
            devices.append(path)

    return sorted(devices, key=lambda path: int(path.removeprefix("/dev/video")))


def annotate(frame, lines: list[str]) -> None:
    y = 32
...
def main() -> int:
    ...
    print("Cover or wave at one physical camera, rerun, and compare images.")
