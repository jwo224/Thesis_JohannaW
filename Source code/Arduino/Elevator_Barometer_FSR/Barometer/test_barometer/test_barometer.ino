...
void addOrUpdateFloor(int floorNumber) {
  ...
  if (floor0Set) {
    ...
    Serial.println("Calibration point will not be logged until floor0 exists.");
  }
...
void finishCalibration() {
  ...
  Serial.println("Live output and CSV logging started.");
...
void setup() {
  ...
  Serial.println();
  Serial.println("ESP32-C3 Floor Detection + Testing + CSV Logging");
  Serial.println("Make sure: Tools -> USB CDC On Boot -> Enabled");
  Serial.println();

  if (!LittleFS.begin(true)) {
    Serial.println("LittleFS mount failed.");
    Serial.println("CSV logging will not work.");
  } else {
...
void loop() {
  ...
  if (now - lastPrintTime >= PRINT_INTERVAL_MS) {
    ...
    // Continuous CSV logging without manual actual-floor comparison
