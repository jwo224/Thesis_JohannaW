...
void setCalibrationFromCSV(int floorNumber, float pressurePa) {
  if (!barometerInitialized) {
    Serial.println("Barometer not initialized.");
    return;
  }
  ...
  if (calibrationFinished) {
    Serial.println("Calibration is already finished.");
    Serial.println("Type resetcal first before loading CSV calibration.");
    return;
  }
  ...
  Serial.print("Loaded CSV calibration: floor ");
  Serial.print(floorNumber);
  Serial.print(", pressure ");
  Serial.print(pressurePa, 2);
  Serial.println(" Pa");
...
void finishCalibration() {
  ...
  if (!floor0Set) {
    ...
    Serial.println("Go to floor 0 and type floor0, or use setcal 0 PRESSURE_PA.");
    return;
  }
...
void resetCalibration() {
  ...
  Serial.println("Set floors again with floor0, floor1, floor2");
  Serial.println("or load from CSV with setcal 0 PRESSURE_PA, setcal 1 PRESSURE_PA, ...");
...
void printBarometerHelp() {
  ...
  Serial.println("  setcal 0 100944.00     - load floor calibration from CSV pressure");
  Serial.println("  setcal 1 100906.50     - load floor calibration from CSV pressure");
...
void handleBarometerCommand(String cmd) {
  ...
  if (cmd.startsWith("setcal")) {
    int firstSpace = cmd.indexOf(' ');
    int secondSpace = cmd.indexOf(' ', firstSpace + 1);

    if (firstSpace < 0 || secondSpace < 0) {
      Serial.println("Invalid setcal command.");
      Serial.println("Use: setcal FLOOR PRESSURE_PA");
      Serial.println("Example: setcal 0 100944.00");
      return;
    }

    int floorNumber = cmd.substring(firstSpace + 1, secondSpace).toInt();
    float pressurePa = cmd.substring(secondSpace + 1).toFloat();

    if (pressurePa <= 0.0) {
      Serial.println("Invalid pressure value.");
      return;
    }

    setCalibrationFromCSV(floorNumber, pressurePa);
    return;
  }
